...
def render_hotwater_view(df: pd.DataFrame) -> None:
    st.header("🌡️ 온수 사용 내역 분석")

    buildings = sorted(df["building"].unique())
    sel_bld = st.multiselect(
        "건물 선택", ["All"] + buildings, default=["All"], key="hw_bld"
    )
    ...
    mc[4].metric("평균 단가",   f"{(df['fee_excl'].sum()/df['usage_m3'].replace(0,np.nan).sum()):,.0f} 원/m³" if df['usage_m3'].sum() > 0 else "-")
    ...
    _metered = df["usage_m3"] > 0
    _upm2    = (df["usage_m3"] / df["size_m2"].replace(0, np.nan)).where(_metered)
    ...
    tab_rank, tab_fair, tab_usage, tab_anom = st.tabs(
        ["순위", "면적당 비용", "사용량", "이상 탐지"]
    )
    ...
    with tab_rank:
        _metric = st.radio(
            "기준", ["총부과", "사용량 (m³)"], horizontal=True, key="hw_rank_metric"
        )
    ...
    with tab_usage:
        st.caption(
            f"계량 브랜드: **{n_metered}개**  |  미계량: **{n_unmet}개**"
        )
        ...
        with _bc1:
            ...
            fig_bu.update_layout(
                title="건물별 총 사용량 (m³)", height=300, plot_bgcolor="white",
        ...
        if _uview == "사용량 순위":
            ...
            for bld in ["A", "B", "C", "D"]:
                ...
                fig_u.add_trace(go.Bar(
                    x=sub["usage_m3"].values,
                    y=[_pfx(b) + str(b)[:26] for b in sub["brand"]],
                    name=f"{bld}동", orientation="h",
            ...
            fig_u.update_layout(
                ...
                xaxis_title="m³", barmode="overlay", plot_bgcolor="white",
                xaxis=dict(gridcolor="#DDDDDD", griddash="dot"),
            ...
            _xf = np.linspace(_xa.min(), _xa.max(), 100)
            fig_s.add_trace(go.Scatter(
    ...
    with tab_anom:
        ...
        if len(_flagged_idx) > 0:
            ...
            _detail = df[df["brand"].isin(_flagged_idx)][
                ["brand", "building", "floor", "size_m2", "usage_m3", "fee_excl", "total"]
            ].copy()
